...
export async function depositForBurnWithHook(params: {
  ...
  // the burn's gas estimate can race a lagging node that still reads allowance 0 → depositForBurn's
  ...
  for (let i = 0; ; i++) {
    const seen = (await pub.readContract({
      address: src.usdc, abi: ERC20_APPROVE_ABI, functionName: 'allowance', args: [account.address, TOKEN_MESSENGER_V2],
    })) as bigint;
