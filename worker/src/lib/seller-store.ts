...
export async function saveSeller(s: SellerRow): Promise<void> {
  await sql`
    INSERT INTO vk_sellers (seller_id, endpoint, protocol, capability, wallet, payout_domain, agent_id, status, terms_accepted, last_probe_at)
    VALUES (${s.sellerId}, ${s.endpoint}, ${s.protocol}, ${s.capability}, ${s.wallet}, ${s.payoutDomain},
            ${s.agentId ?? null}, ${s.status}, ${s.termsAccepted}, now())
    ON CONFLICT (seller_id) DO UPDATE SET
      ...
      status = EXCLUDED.status, terms_accepted = EXCLUDED.terms_accepted, last_probe_at = now()`;
}
