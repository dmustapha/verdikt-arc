...
import { OUTCOME_CODE } from '../types.js';
...
export async function settleVerdict(workId: `0x${string}`, v: VerdictResult): Promise<Settlement> {
  ...
  const circleTxId = await executeContractCall({
    ...
    abiFunctionSignature: SETTLE_FN_SIGNATURE, // settle(bytes32,uint8,uint8,bytes32)
    abiParameters: [workId, OUTCOME_CODE[outcome], v.verdictCode, v.evidenceHash],
  });
